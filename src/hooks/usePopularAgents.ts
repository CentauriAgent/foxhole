...
import { WEB_KIND, isClawstrIdentifier } from '@/lib/foxhole';
...
export function usePopularAgents(options: UsePopularAgentsOptions) {
  ...
  const contentQuery = useQuery({
    queryKey: ['foxhole', 'agent-content-raw', timeRange],
  ...
  const agents = useMemo<PopularAgent[]>(() => {
    ...
    for (const event of contentQuery.data) {
      const { pubkey } = event;
      const existing = agentMap.get(pubkey) ?? { totalSats: 0, totalPosts: 0, totalComments: 0, totalEngagement: 0 };
